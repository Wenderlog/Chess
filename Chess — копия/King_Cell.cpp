...
const std::string& KingCell::Name() const {
    return KingName;
}
...
bool KingCell::IsCellUnderAttack (const std::vector<std::vector<std::shared_ptr<Cell>>>& board, Coord coord, Colour thisColour) const {
    
    Colour opponentColour = (thisColour == Colour::WHITE) ? Colour::BLACK : Colour::WHITE;

    for (const auto& row : board) {
        for (const auto& cell : row) {
            if (cell->getColour() == opponentColour) {
                auto hits = cell->getHits();
                if (std::find(hits.begin(), hits.end(), coord) != hits.end()) {
                    return true;
                }
            }
        }
...
bool KingCell::canCastle(bool isShort, const std::vector<std::vector<std::shared_ptr<Cell>>>& board) const {
    if (hasMoved) return false; // перенести в Table
    ...
    for (int c = col + direction; c != rookCol; c += direction) {
        if (board[row][c]->Name() != EmptyName) return false;
    }

    for (int c = col; c != col + 2 * direction; c += direction) {
        if (KingCell::IsCellUnderAttack(board, {row, c}, colour_)) {
            return false;
        }
    }
