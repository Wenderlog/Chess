...
public:
    ...
    enum class TurnVerdict : std::uint8_t // underlying type
    {
    ...
    void PromotePawn(Coord position, char promotionType); // переписать также как TurnVerdict enum class

    /*!
     * \brief Parses and executes a move described in algebraic notation.
     * \details This function is a friend of the `Table` class, allowing it to access private members.
     * \param table The chessboard instance.
     * \param message The move in algebraic notation (e.g., "e2 e4").
     * \return A pair of coordinates representing the move.
     */
    
    friend std::pair<Coord, Coord> WordToCoord(Table& table, const std::string& message); // переписать без friend
