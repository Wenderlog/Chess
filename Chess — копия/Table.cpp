...
bool Table::CheckAttack(Coord from, Coord to) const {
    ...
    if (toCell && fromCell->getColour() == toCell->getColour()) {
        return false;
    }
    ...
    if (fromCell->Name() == PawnName) {
        ...
        if ((last_move_is_w_pawn_ || last_move_is_b_pawn_) &&
            coord_w_pawn_ == Coord(to.row - direction, to.col)) {
            if (toCell == nullptr && abs(from.col - to.col) == 1 && to.row == from.row + direction) {
                return true;
            }
        }
        
        if (toCell && abs(from.col - to.col) == 1 && to.row == from.row + direction) {
            return true;
        }
...
Table::TurnVerdict Table::CheckTurn(Coord from, Coord to) const {
    ...
    //переписать, у нас все ходы валидны
}
...
void Table::PromotePawn(Coord position, char promotionType)// переписать char-> enum class
{
    ...
    std::cout << "Pawn at (" << position.row << ", " << position.col
              << ") promoted to " << promotionType << ".\n";
}
