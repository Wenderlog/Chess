...
constexpr std::string KingName = "King";
...
public:
    ...
    /*!
     ...
     \return A vector of coordinates representing the cells the king can attack.
     */
    ...
    /*!
     ...
     \return A vector of coordinates representing the cells the king can move to.
     */
    ...
    /*!
     ...
     \param thisColour The color of the king performing the check.
     \return `true` if the cell is under attack; otherwise, `false`.
     */
    bool IsCellUnderAttack(const std::vector<std::vector<std::shared_ptr<Cell>>>& board, Coord coord, Colour thisColour) const;
